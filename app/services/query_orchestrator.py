...
class QueryOrchestrator:
    ...
    async def execute(
        ...
        icrm_token: Optional[str] = None,   # ICRM admin token from user_sso_tokens DB via MARS
    ) -> OrchestratorResult:
        """
        ...
        if seller_token:
            ...
            logger.info("orchestrator.seller_token_injected",
                        company_id=cid, seller_token_injected=True)

        # Inject ICRM admin token for /v1/admin/* API calls (generated by MARS on session create).
        if icrm_token:
            if session_context is None:
                session_context = {}
            session_context["icrm_token"] = icrm_token
            logger.info("orchestrator.icrm_token_injected",
                        company_id=cid, icrm_token_injected=True)
        ...
        # ---------------------------------------------------------------
        # PARAM CLARIFICATION GATE (Phase 6 — vector-search driven)
        # Driven by knowledge_chunks from vector search, NOT by intent name
        # string matching. This handles any user phrasing — semantic search
        # already found the right API doc regardless of exact wording.
        #
        # If the top matched chunk belongs to an API with soft_required_context
        # and a required param is missing → ask ONE targeted question.
        # ---------------------------------------------------------------
        if not result.needs_clarification:
            try:
                from app.brain.param_clarifier import ParamClarificationEngine
                kb_root = getattr(self, "_kb_root_for_clarifier", None)
                if kb_root:
                    clarifier = getattr(self, "_param_clarifier", None)
                    if clarifier is None:
                        clarifier = ParamClarificationEngine(kb_root)
                        self._param_clarifier = clarifier
                    knowledge_chunks = (
                        result.context.get("knowledge_chunks", [])
                        if isinstance(result.context, dict) else []
                    )
                    clarification_req = await clarifier.check(
                    ...
                    if clarification_req:
                        result.needs_clarification = True
                        result.clarification_prompt = clarification_req.question
                        result.response_metadata = {
                            "resolution_tier": 0,
                            "reason": "param_clarification",
                            "missing_param": clarification_req.pending_param,
                            "api_entity_id": clarification_req.api_entity_id,
                            "similarity": clarification_req.similarity,
                        }
                        result.total_latency_ms = (time.monotonic() - total_start) * 1000
                        logger.info(
                            "orchestrator.param_clarification_gate_fired",
                            missing_param=clarification_req.pending_param,
                            api=clarification_req.api_entity_id,
                            similarity=round(clarification_req.similarity, 3),
                        )
                        return result
            except Exception as _ce:
                logger.debug("orchestrator.param_clarifier_error", error=str(_ce))
        ...
        if gate1.decision == TierDecision.RESPOND:
            ...
            self.cost_tracker.finalize(cost_record)
            result.total_latency_ms = (time.monotonic() - total_start) * 1000
            self._apply_confidence_gate(result, entity_resolved, query)
            return result
    ...
    def _apply_confidence_gate(
        self, result: "OrchestratorResult", entity_resolved: bool, query: str
    ) -> None:
        """Compute weighted confidence score and set confidence_gate in response_metadata.

        Called in every return path so the gate fires regardless of which tier responded.
        """
        try:
            from app.engine.confidence import score_confidence

            _total_attr = len(result.attributions)
            _successful_attr = sum(1 for a in result.attributions if a.found_data)
            _tool_success_rate = (_successful_attr / _total_attr) if _total_attr > 0 else 0.0

            _result_completeness = (
                result.relevant_items / result.total_items
                if result.total_items > 0 else 0.0
            )

            _intent_clarity = float(
                (result.request_classification or {}).get("confidence", 0.5)
            )

            _entity_resolved_score = 1.0 if entity_resolved else 0.0
            ...
            final_confidence = score_confidence(
                ...
                entity_match=_entity_resolved_score,
            )

            if result.response_metadata is None:
                result.response_metadata = {}
            result.response_metadata["final_confidence"] = round(final_confidence, 3)
            ...
            if final_confidence < 0.3:
                ...
                result.response_metadata["confidence_gate"] = "refused"
                logger.info(
                    "orchestrator.confidence_gate.refused",
                    confidence=round(final_confidence, 3),
                    query=query[:60],
                )
            elif final_confidence < 0.6:
                result.response_metadata["confidence_gate"] = "uncertain"
                result.response_metadata["uncertainty_marker"] = True
                logger.info(
                    "orchestrator.confidence_gate.uncertain",
                    confidence=round(final_confidence, 3),
                    query=query[:60],
                )
            else:
                result.response_metadata["confidence_gate"] = "confident"
        except Exception as _cg_err:
            logger.debug("orchestrator.confidence_gate_error", error=str(_cg_err))
    ...
    async def _wrap_probe_with_progress(
        self, key: "PipelineName", coro
    ) -> "ProbeResult":
        """Wrap a probe coroutine to emit per-task SSE progress events."""
        await self._emit_wave_progress(1, f"probe_{key.value}", "running", {})
        try:
            probe_result = await coro
            await self._emit_wave_progress(
                1, f"probe_{key.value}", "completed",
                {"found": probe_result.found_data,
                 "latency_ms": round(getattr(probe_result, "latency_ms", 0.0), 1)},
            )
            return probe_result
        except Exception as exc:
            await self._emit_wave_progress(
                1, f"probe_{key.value}", "failed", {"error": str(exc)}
            )
            raise
    ...
    async def _stage1_parallel_probe(
        ...
        # Wrap each probe to emit per-task SSE progress, then apply 10s timeout
        wrapped = [
            asyncio.wait_for(self._wrap_probe_with_progress(k, c), timeout=10.0)
            for k, c in zip(keys, coros)
        ]
    ...
    async def _enrich_query_with_claude(
        ...
        try:
            ...
            api_key = settings.AIGATEWAY_API_KEY or ""
            api_url = settings.AIGATEWAY_URL
            if not api_key:
                return None
